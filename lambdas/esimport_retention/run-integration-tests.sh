...
bash ./deploy-s3-bucket.sh ${s3_stack_name} \
	${s3_bucket_name}
...
python -m pytest tests -x --cov=esimport_retention_core --cov=esimport_snapshot_creation --cov=esimport_snapshot_verifier --cov=esimport_retention --cov-report=term-missing
