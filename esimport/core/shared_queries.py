...
WHERE Org_Value.Organization_ID = ? AND Name NOT IN ('EradApiKey')"""


"""
The following query returns the owning service provider for the given org.  If there are multiple
service providers in the org lineage, then the most distant ancestor will be used.
"""
GET_PROVIDER_QUERY = """
SELECT TOP 1 Organization.Display_Name as Provider
FROM Org_Relation_Cache WITH (NOLOCK)
    JOIN Organization ON Organization.ID = Org_Relation_Cache.Parent_Org_ID
WHERE 
    Org_Relation_Cache.Child_Org_ID = ?
...
SELECT COUNT(DISTINCT r.Member_ID) as ActiveMembers,
    COUNT(DISTINCT r.Calling_Station_Id) as ActiveDevices
