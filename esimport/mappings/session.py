...
class SessionMapping(PropertyAppendedDocumentMapping):
    ...
    def process_sessions_from_id(self, next_id_to_process: int, start_date, use_historical: bool) -> (int, int, datetime):
        ...
        for session in self.model.get_sessions(next_id_to_process, self.db_record_limit, start_date, use_historical):
            ...
            next_id_to_process = session.get('ID') + 1
    ...
    def sync(self, start_date):
        ...
        while True:
            count, next_id_to_process, most_recent_session_time = self.process_sessions_from_id(next_id_to_process, start_date, use_historical)
